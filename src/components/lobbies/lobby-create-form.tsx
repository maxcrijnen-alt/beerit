...
export function LobbyCreateForm({
  ...
  const defaultMode = defaults?.activitySelectionMode ?? "MIXED";
  const defaultKinds = new Set(defaults?.activityKinds ?? []);
  const mixedCategories = defaults?.mixedCategories ?? [];

  const [selectionMode, setSelectionMode] = useState(defaultMode);
  const [checkedKinds, setCheckedKinds] = useState<Set<GameActivityKind>>(defaultKinds);
        ...
        <label className="flex min-h-11 items-start gap-2 rounded-md py-1 text-xs">
          ...
          <span>
            ...
            <span className="mt-1 block leading-5 text-muted-foreground">
              ...
              only dice games, or your own combination. You must check at least
              one game type below.
            </span>
          </span>
        </label>
      ...
      >
        ...
        {onlySelectedWithNoKinds ? (
          <p className="text-xs text-destructive">
            Select at least one game type when using &ldquo;Only selected offline games&rdquo;.
          </p>
        ) : null}
      </fieldset>
      {state.message ? (
