...
- `codex/game-topics-schema`: Codex — game topics schema work. Do not
  touch `supabase/migrations/`, `src/types/database.ts`,
  `src/app/games/social-actions.ts`, `src/lib/validation/games.ts`,
  `src/lib/games/queries.ts`, or
  `src/components/games/community-question-form.tsx` until merged.
